...
class BackgroundService {
  ...
  private handleCommand(command: string, tab?: chrome.tabs.Tab) {
    if (!tab?.id) return;
  ...
  private async simplifyText(text: string, options: any = {}) {
    try {
  ...
  private async translateText(text: string, options: any = {}) {
    try {
  ...
  private async rewriteText(text: string, options: any = {}) {
    try {
  ...
  private async proofreadText(text: string, options: any = {}) {
    try {
  ...
  private async readAloud(text: string, options: any = {}) {
    try {
