...
class ContentScriptAI {
    ...
    async proofreadSelection() {
        try {
            ...
            if (result.success && result.data) {
                ...
                this.showNotification(correctionCount > 0 ? `✅ Found ${correctionCount} correction(s)` : '✅ No errors found!', correctedText.substring(0, 100) + (correctedText.length > 100 ? '...' : ''));
                return result;
            }
            else {
    ...
    async translateSelection(targetLanguage = 'es') {
        try {
            const selectedText = this.getSelectedText();
            if (!selectedText) {
                this.showNotification('ℹ️ No text selected', 'Please select some text first');
    ...
    async rewriteSelection(options) {
        try {
            const selectedText = this.getSelectedText();
            if (!selectedText) {
                this.showNotification('ℹ️ No text selected', 'Please select some text first');
